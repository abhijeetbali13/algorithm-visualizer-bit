...
export default function LCS() {
  ...
  const { s1: ps1, s2: ps2 } = PRESETS[presetIdx];
  const s1 = activeStrings ? activeStrings[0] : ps1;
  const s2 = activeStrings ? activeStrings[1] : ps2;
  ...
  const switchPreset = (i) => { viz.reset(); setPresetIdx(i); setActiveStrings(null); setCustomS1(''); setCustomS2(''); setCustomError(''); };

  const applyCustom = () => {
    const a = customS1.trim().toUpperCase();
    const b = customS2.trim().toUpperCase();
    if (!a || !b) { setCustomError('Both strings required'); return; }
    if (a.length > 12 || b.length > 12) { setCustomError('Max 12 chars per string for readability'); return; }
    setCustomError(''); viz.reset(); setActiveStrings([a, b]);
  };
  ...
  return (
    <div className="algo-page">
      <div className="page-wrapper">
        ...
        <AlgoTabWrapper algoId="lcs" meta={META} steps={steps||[]} stepIdx={stepIdx||-1} currentMsg={current?.msg||''} onJumpTo={jumpTo}>

        <div className="algo-layout">
          <div>
            {/* Custom input */}
            <div className="controls-panel" style={{ marginBottom: 14 }}>
              <h3>Custom Strings</h3>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4, fontFamily: 'JetBrains Mono' }}>String 1</div>
                  <input type="text" placeholder="e.g. ABCBDAB" value={customS1} onChange={e => setCustomS1(e.target.value)}
                    style={{ width: 120, padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontFamily: 'JetBrains Mono', fontSize: 13, textTransform: 'uppercase' }} />
                </div>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4, fontFamily: 'JetBrains Mono' }}>String 2</div>
                  <input type="text" placeholder="e.g. BDCAB" value={customS2} onChange={e => setCustomS2(e.target.value)}
                    style={{ width: 120, padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontFamily: 'JetBrains Mono', fontSize: 13, textTransform: 'uppercase' }} />
                </div>
                <button className="btn btn-primary" onClick={applyCustom} style={{ padding: '6px 14px', fontSize: 12 }}>Apply</button>
                {activeStrings && <button className="btn btn-secondary" onClick={() => { setActiveStrings(null); setCustomS1(''); setCustomS2(''); viz.reset(); }} style={{ padding: '6px 10px', fontSize: 11 }}>Clear</button>}
              </div>
              {customError && <div style={{ color: 'var(--red)', fontSize: 12, marginTop: 4 }}>{customError}</div>}
              {activeStrings && <div style={{ fontSize: 11, color: 'var(--green)', marginTop: 4 }}>✓ Using custom: "{activeStrings[0]}" / "{activeStrings[1]}"</div>}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {current && (
              <div className="prog-chips">
                {[['Matches', steps.slice(0, stepIdx + 1).filter(s => s.match).length], ['Steps', stepIdx+1], ['Progress', `${Math.round((stepIdx+1)/steps.length*100)}%`]].map(([l,v])=>(
            ...
            <div className="controls-panel">
              <h3>Java Code</h3>
              <div className="pseudocode" style={{ fontSize: 11 }}>{`public static int lcs(
    String s1, String s2) {
